"use client";

import { TriangleAlert } from "lucide-react";

export default function DangerModal() {
  return (
    <div className="z-10  flex items-center justify-center">
      <div className="flex items-end justify-center text-center sm:items-center sm:p-0 overflow-hidden">
        <div className="relative transform p-8 overflow-hidden rounded-lg text-left transition-all data-closed:translate-y-4 data-closed:opacity-0 data-enter:duration-300 data-enter:ease-out data-leave:duration-200 data-leave:ease-in  sm:w-full sm:max-w-lg sm:p-6 data-closed:sm:translate-y-0 data-closed:sm:scale-95">
          <div className="sm:flex sm:items-start">
            <div data-color="error" className="mx-auto flex size-12 shrink-0 items-center justify-center rounded-full bg-error/10 sm:mx-0 sm:size-10">
              <TriangleAlert aria-hidden="true" className="size-6 text-error" />
            </div>
            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 data-color="text" className="text-base font-semibold text-text">
                Delete your color palette?
              </h3>
              <div className="mt-2">
                <p data-color="muted" className="text-sm text-muted">
                  All those hours of &quot;is this coral or salmon?&quot; will vanish into
                  the void. Your gradients will weep. This cannot be undone.
                </p>
              </div>
            </div>
          </div>
          <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
            <button
              type="button"
              //   onClick={() => setOpen(false)}
              data-color="error"
              className="inline-flex w-full justify-center rounded-md bg-error px-3 py-2 text-sm font-semibold text-onError hover:bg-error/90 sm:ml-3 sm:w-auto"
            >
              Yes, delete it
            </button>
            <button
              type="button"
              data-autofocus
              //   onClick={() => setOpen(false)}
              data-color="secondary"
              className="mt-3 inline-flex w-full justify-center rounded-md bg-secondary px-3 py-2 text-sm font-semibold text-onSecondary ring-1 ring-border ring-inset hover:bg-secondary/90 sm:mt-0 sm:w-auto"
            >
              Cancel
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
